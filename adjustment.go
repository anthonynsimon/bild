...
func Equalize(img image.Image) *image.RGBA {
	...
	fn := func(h Histogram, min, value int) int {
		return (((h.Bins[value] - min) << 16 / ((srcW * srcH) - min)) * 255) >> 16
	}

	rMin, gMin, bMin := hist.R.Min(), hist.G.Min(), hist.B.Min()

	for x := 0; x < srcW; x++ {
		for y := 0; y < srcH; y++ {
			pos := y*src.Stride + x*4
			dst.Pix[pos+0] = uint8(fn(hist.R, rMin, int(src.Pix[pos+0])))
			dst.Pix[pos+1] = uint8(fn(hist.G, gMin, int(src.Pix[pos+1])))
			dst.Pix[pos+2] = uint8(fn(hist.B, bMin, int(src.Pix[pos+2])))
